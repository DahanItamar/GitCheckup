...
**[M5] `repo_aliases` exists because GitHub never breaks an old slug.** `facebook/react` still resolves — as a `301` to `react/react` — years after the transfer. `fetch` follows it, the API reports the canonical identity, and `repos` stores that. A lookup by the _requested_ slug therefore matched nothing, so every request for a renamed repository was a cold six-call fan-out, permanently: scoring it again never made the next lookup match. Worst on `/api/og` and `/api/badge`, which GitHub's Camo proxy calls on every README view — a README embedding an old slug would have spent six calls of the 5000/hr budget per cache miss with nobody waiting on the result (§7 Flow B). `findLatestScore` now matches the current slug **or** any alias in one predicate — two queries would double the latency of the commonest read to serve the rarest case — and an alias row is written only when a requested slug turns out to differ from the canonical one. Measured against live Neon: 1.63s → **0.064s**. Found on the first run against a real database, not by reading the code.
