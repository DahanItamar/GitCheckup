...
| What ranks on `/improved`             | `lib/db/scores.ts` → `findMostImproved` — by gain, never by absolute score   |
| The two-repo comparison               | `components/CompareRadar.tsx` + `CompareBars.tsx`; `lib/services/compare.ts` |
